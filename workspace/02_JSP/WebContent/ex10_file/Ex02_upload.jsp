...
<body>
	<%
		...
		// 2. 파일이 업로드 될 디렉터리의 실제경로를 변수로 저장한다.
		...
		MultipartRequest multipart = new MultipartRequest(
				...
				1024 * 1024 * 10, // 10MB (1024바이트 == 1KB, 1024kB == 1MB)
				"utf-8", // 인코딩
				new DefaultFileRenamePolicy()  // 기존파일명변경정책
				);		
	%>
	...
	<ul>
		...
		<%
			...
			long size = file.length() / 1024;  // lenght() 메소드는 바이트를 반환하므로 1024로 나누어 KB로 저장
			String date = new SimpleDateFormat("yyyy-MM-dd a h:mm").format(file.lastModified());
		%>
		<li>저장된파일명: <%=filename%></li>
		<li>파일 크기: <%=size %>KB</li>
		<li>수정한 날짜 : <%=date%></li>
	</ul>
	
	<%-- 다운로드 링크 --%>
	<a href="/02_JSP/ex10_file/Ex03_download.jsp?directory=
	<%=directory%>&filename=<%=multipart.getOriginalFileName("filename") %>">다운로드</a>
